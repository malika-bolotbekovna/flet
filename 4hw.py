...
def main(page: ft.Page):
    ...
    hide_show = ft.TextButton("Скрыть историю")  # пока без on_click

    def hide_show_button(_):
        nonlocal history_visible
        history_visible = not history_visible
        history_column.visible = history_visible
        hide_show.text = "Скрыть историю" if history_visible else "Показать историю"
        page.update()

    hide_show.on_click = hide_show_button  # связываем кнопку с функцией
